...
namespace APChangeKitAPI.Common
{
    ...
    public class HttpHelper
    {
        ...
        public HttpResult Execute(HttpItem item)
        {
            //返回参数
            ...
            catch (Exception ex)
            {
                ...
                //配置参数时出错
                return new HttpResult() { Cookie = string.Empty, Header = null, Result = ex.Message, StatusDescription = "配置参数时出错：" + ex.Message };
            }
            try
            {
                //请求数据
            ...
            if (item.IsToLower) result.Result = result.Result.ToLower();
